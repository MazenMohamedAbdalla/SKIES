...
public class user {
    ...
    public boolean addBankAccount(String cardNumber, String cardHolder, String expDate, String cvv) {
        ...
        try {
            ...
            if (query.next()) {
                ...
                int query2 = stm.executeUpdate("INSERT INTO skies.bank_accounts VALUES ('" + account.getCardNumber() + "','" + getId() + "','" + account.getBalance() + "');");
                if (query2 > 0) {
                    stm.close();
                    added = true;
                }
    ...
    // VIEW BACCOUNT DONE !
    public ArrayList<String> viewBankAccount() {
    ...
    public ArrayList<String> searchFlight(String source, String destination, String departureDate, String flightClass) {
        ArrayList<String> flightResults = new ArrayList<String>();
        ...
        try {
            Statement stm = myDb.createStatement();
            ResultSet query = stm.executeQuery("SELECT * FROM skies.one_way_flights \r\n"
            ...
            while (query.next()) {
                flightResults.add(query.getString("flight_number")); // flightNumber
                flightResults.add(query.getString("departure")); // departure
                flightResults.add(query.getString("arrival")); // arrival
                flightResults.add(query.getString("duration")); // duration
                flightResults.add(query.getString("class")); // class
                flightResults.add(query.getString("Cost")); // cost
            }
            stm.close();
        } catch (Exception e) {
            Log.e("Error",e.getMessage());
    ...
    public boolean flightStatus(String flightNumber) {
        ...
        try {
            ...
            if (query.next()) {
                ...
                flight.setDepartureTime(query.getString("TIME(departure)"));
                flight.setDepartureDate(query.getString("DATE(departure)"));
                flight.setArrivalTime(query.getString("TIME(arrival)"));
    ...
    public boolean bookFlight(String flightNumber, String passengerName, String cardNumber) {
        ...
        try {
            ...
            if (numberOfSeats > 0 && accBalance >= ticketCost) {
                ...
                if (insertQuery > 0 && updateBalance > 0 && decNumberOfSeats > 0 && checkBooked > 0) {
    ...
    public boolean cancelFlight(String flightNumber, String passengerName) {
        ...
        try {
            ...
            if (bookedTicket.next()) {
                ...
                if (cancelQuery > 0 && updateBalance > 0 && decNumberOfSeats > 0 && checkBooked > 0) {
                    canceled = true;
                }
                // -----
            }
    ...
    // MYBOOKINGS DONE !
    public ArrayList<String> myBookings() {
        ...
        try {
            ...
            ResultSet query = stm.executeQuery("SELECT passenger_name,ticket.flight_number,ticket.cost,seat_number,source_from,destination_to,TIME(departure),DATE(departure),TIME(arrival),DATE(arrival),class\r\n"
                    ...
                    + "WHERE user_id = '" + getId() + "';");
            while (query.next()) {
                bookings.add(query.getString("passenger_name"));
                bookings.add(query.getString("ticket.flight_number"));
                bookings.add(Float.toString(query.getFloat("ticket.cost"))); // ticket.cost
                bookings.add(query.getString("seat_number")); // seat_number
                bookings.add(query.getString("source_from")); // source_from
                bookings.add(query.getString("destination_to")); // destination_to
                bookings.add(query.getString("DATE(departure)")); // departure DATE
                bookings.add(query.getString("TIME(departure)")); // departure TIMR
                bookings.add(query.getString("DATE(arrival)"));// arrival DATE
                bookings.add(query.getString("TIME(arrival)")); // arrival TIME
                bookings.add(query.getString("class")); // class
            }
            stm.close();
        } catch (Exception e) {
            Log.e("Error",e.getMessage());
